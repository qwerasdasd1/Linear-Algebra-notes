...
\begin{definitionenv}
	...
	\end{theoremenv}
	\begin{corollaryenv}
		$0_\mathcal{K}$ and $1_\mathcal{K}$ in \textbf{Field Axiom 2} is unique.
	\end{corollaryenv}
	\begin{exampleenv}
...
\begin{definitionenv}
	...
	\begin{enumerate}
		\item \textbf{Associative Laws:} for any $\bm{u}, \bm{v}, \bm{w} \in V$,
		\[
		\bm{u} + (\bm{v} + \bm{w}) = (\bm{u} + \bm{v}) + \bm{w}.
		\]

		\item \textbf{Existence of Identity Elements:}
		there exists a zero vector $\bm{0} \in V$ such that for any $\bm{v} \in V$,
		\[
		\bm{v} + \bm{0} = \bm{v}.
		\]

		\item \textbf{Existence of Additive Inverses:}
		for any $\bm{v} \in V$, there exists an element $-\bm{v} \in V$ such that
		\[
		\bm{v} + (-\bm{v}) = \bm{0}.
		\]

		\item \textbf{Commutative Law:}
		for any $\bm{u}, \bm{v} \in V$,
		\[
		\bm{u} + \bm{v} = \bm{v} + \bm{u}.
		\]

		\item \textbf{Distributive Laws:}
		for all $a, b \in \mathcal{K}$ and $\bm{u}, \bm{v} \in V$,
		\[
		a \cdot (\bm{u} + \bm{v}) = a \cdot \bm{u} + a \cdot \bm{v},
		\quad (a + b) \cdot \bm{v} = a \cdot \bm{v} + b \cdot \bm{v}.
		\]

		\item \textbf{Compatibility of Scalar Multiplication:}
		for all $a, b \in \mathcal{K}$ and $\bm{v} \in V$,
		\[
		(a b) \cdot \bm{v} = a \cdot (b \cdot \bm{v}).
		\]

		\item \textbf{Identity of Scalar Multiplication:}
		for all $\bm{v} \in V$,
		\[
		1_{\mathcal{K}} \cdot \bm{v} = \bm{v}.
		\]
	\end{enumerate}
\end{definitionenv}
\begin{theoremenv}
	The zero vector $\bm{0}$ is unique.
\end{theoremenv}
\begin{definitionenv}
	...
	Equivalently, $W$ is a subspace of $V$ if and only if the following conditions hold for all $\bm{u}, \bm{v} \in W$ and $a \in \mathcal{K}$:
	\begin{enumerate}
...
\end{notationenv}
\begin{remarkenv}
	We state $W$ to be a non-empty set at first so we needn't to say $\bm{0}\in W$.
\end{remarkenv}
\begin{remarkenv}
...
\begin{definitionenv}
	...
	$\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n \in V$.
	Any vector of the form
	\[
	\bm{v} = a_1 \bm{v}_1 + a_2 \bm{v}_2 + \cdots + a_n \bm{v}_n,
	\]
...
\begin{definitionenv}
	...
	$S = \{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\} \subseteq V$.
	The set of all linear combinations of vectors in $S$ is called the
	\textbf{span} of $S$, denoted by
	\[
	\mathrm{span}(S) =
	\left\{
	a_1 \bm{v}_1 + a_2 \bm{v}_2 + \cdots + a_n \bm{v}_n
	\ \middle| \ a_1, a_2, \dots, a_n \in \mathcal{K}
	\right\}.
	\]
\end{definitionenv}
\begin{theoremenv}
	The span of $S$ is the smallest subspace of $V$ containing $S$. Particularly, span$(\varnothing)=\{\bm{0}\}=\text{span}\{\bm{0}\}$.
\end{theoremenv}
\begin{definitionenv}
	Let $V$ be a vector space over a field $\mathcal{K}$.
...
\end{definitionenv}
\begin{remarkenv}
	$\{\bm{0}\}$ is linearly dependent, but $\varnothing$ is linearly independent.
\end{remarkenv}
...
\begin{definitionenv}
	Let $V$ be a vector space over a field $\mathcal{K}$.
	A subset $\mathcal{B} = \{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\} \subseteq V$ is called a \textbf{basis} of $V$ if it satisfies the following two conditions:
	\begin{enumerate}
...
\begin{theoremenv}
	Let $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\}$ be a set of elements of a vector space $V$ over a field $\mathcal{K}$, and let $r$ be a positive integer with $r \leq n$.
	We say that $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_r\}$ is a \textbf{maximal subset of linearly independent elements} if
	$\bm{v}_1, \dots, \bm{v}_r$ are linearly independent, and moreover, for any $\bm{v}_i$ with $i > r$, the set
	$\{\bm{v}_1, \dots, \bm{v}_r, \bm{v}_i\}$ is linearly dependent.

	If, in addition, $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\}$ is a set of generators of $V$,
	then the set $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_r\}$ is a \textbf{basis} of $V$.
\end{theoremenv}
...
\begin{theoremenv}
	Let $V$ be a vector space over a field $\mathcal{K}$.
	Let $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_m\}$ be a basis of $V$ over $\mathcal{K}$.
	Let $\bm{w}_1, \bm{w}_2, \dots, \bm{w}_n$ be elements of $V$, and assume that $n > m$.
	Then $\bm{w}_1, \bm{w}_2, \dots, \bm{w}_n$ are linearly dependent.
\end{theoremenv}
...
\begin{definitionenv}
	...
	$\{\bm{e}_1, \bm{e}_2, \dots, \bm{e}_n\}$ be an ordered basis of $V$.
	For any vector $\bm{v} \in V$, there exist unique scalars
	$a_1, a_2, \dots, a_n \in \mathcal{K}$ such that
	\[
	\bm{v} = a_1 \bm{e}_1 + a_2 \bm{e}_2 + \cdots + a_n \bm{e}_n.
	\]
	The scalars $a_1, a_2, \dots, a_n$ are called the \textbf{components} or \textbf{coordinates} of $\bm{v}$
	with respect to the basis $\{\bm{e}_1, \bm{e}_2, \dots, \bm{e}_n\}$. And the $n$-tuple $A=(a_1,\dots,a_n)$ is called the \textbf{coordinate vector} of \textbf{v}.
\end{definitionenv}
...
\begin{definitionenv}
	...
	\begin{enumerate}
		\item $\mathcal{B}$ is linearly independent;
		\item for any vector $\bm{v} \in S \setminus \mathcal{B}$, the set
		$\mathcal{B} \cup \{\bm{v}\}$ is linearly dependent.
	\end{enumerate}
\end{definitionenv}

\begin{theoremenv}
	Let $V$ be a vector space over a field $\mathcal{K}$, and let the set
	$\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\}$ be a maximal set of linearly independent elements of $V$.
	Then $\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\}$ is a basis of $V$.
\end{theoremenv}
...
\begin{theoremenv}
	...
	$\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n$ be linearly independent elements of $V$.
	Then $\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n$ constitute a basis of $V$.
\end{theoremenv}
...
\begin{propositionenv}
	...
	$\bm{v}_1, \bm{v}_2, \dots, \bm{v}_r$ be linearly independent elements of $V$.
	Then there exist elements $\bm{v}_{r+1}, \bm{v}_{r+2}, \dots, \bm{v}_n$ in $V$ such that
	\[
	\{\bm{v}_1, \bm{v}_2, \dots, \bm{v}_n\}
	\]
	is a basis of $V$.
